...
from .reshard import reshard_module
...
# ── reshard ───────────────────────────────────────────────────────────────────
@cli.command("reshard", short_help="Split large feather file(s) into smaller shards.")
@click.option(
    "--input_path",
    "-i",
    "input_path",
    ...
    help="A single .feather file, or a folder of .feather files, to reshard.",
)
@click.option(
    "--output_folder",
    "-o",
    ...
    help="Folder to write the resulting shards to (created if missing).",
)
@click.option(
    "--rows_per_shard",
    default=100_000,
    type=int,
    show_default=True,
    help="Target number of simulations per output shard.",
)
@click.option(
    "--prefix",
    default="shard",
    show_default=True,
    help="Filename prefix for output shards, e.g. 'shard_000001.feather'.",
)
def reshard(
    input_path: Path,
    output_folder: Path,
    ...
    """Split one or a few large feather files into many smaller shards.

    Use this to prepare a dataset for ``train --streaming``: that command
    bounds memory by keeping only a handful of *shards* resident at once, so
    it needs a folder of reasonably sized files, not one monolithic file
    containing every simulation. This includes the common case of a dataset
    generated by a single ``mach3sbi simulate`` call before this command
    existed.

    Reads in fixed-size, memory-mapped chunks, so the input file is never
    fully materialised in RAM regardless of its total size.

    Example::

        mach3sbi reshard -i sims.feather -o sims_sharded/ --rows_per_shard 100000
    """
    reshard_module(input_path, output_folder, rows_per_shard, prefix)
...
@optgroup.option(
    ...
    is_flag=True,
    default=False,
    help="Stream simulations from disk instead of loading every shard into "
    "RAM up front. Use this once the full (theta, x) corpus no longer fits "
    "comfortably in memory. Requires --dataset to contain many reasonably "
    "sized shards — see the 'reshard' command if it currently contains one "
    "or a few very large files.",
)
@optgroup.option(
    "--cache_size",
    default=8,
    type=int,
    show_default=True,
    help="Only used with --streaming: number of decoded shards kept "
    "resident per DataLoader worker process. Peak resident shards is "
    "roughly cache_size * num_workers.",
)
# Model architecture
...
def train(
    ...
    For datasets too large to fit in RAM, pass ``--streaming`` to read
    simulations directly from disk instead of loading every shard up front
    — see ``mach3sbi reshard --help`` if ``--dataset`` currently contains
    one or a few very large files rather than many reasonably sized shards.
